...
if nonstat > 0
    fprintf(1, 'initial model nonstationary in varmapqPQestime\n');
    %  ferror=1; return
    ...
    [str, ferror] = suvarmapqPQe(Lambda, alpha, betap, th, Th, Sigma, freq);
    cont = 0; %contf=0;
    for i = 1:nxi
        if xi(i) == 1
            cont = cont + 1;
...
if noninv > 0
    fprintf(1, 'initial model noninvertible in varmapqPQestime\n');
    %  ferror=1; return
...
if (abs(sum(sum(J))) <= 1.d-8)
    error('estimation failed in varmapqPQestime')
end
%end of addition
...
[F, xvf, xf, e, f, h, H, A, P] = lkevarmapqPQe(xvf, y, Y, xf, str, chb, constant);
