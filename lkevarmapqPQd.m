...
[phi, th, Phi, Th, L] = pr2varmapqPQ(xvv, xff, str);

%transform multiplicative to non-multiplicative model
[phirs, thrs, H, F, G, J, ferror] = varmapqPQ2ssm(phi, th, Phi, Th, L, str);

% %initial state covariance matrix
% [Sigma,nonstat] = mlyapunov(F,G*G',.995);
% %check stationarity
% if nonstat > 0
% %  fprintf(1,'model nonstationary, nonstat = %2d\n',nonstat);
%  %initial values are in structure str
%  %we replace initial values with initial values
%  H=str.Z0; F=str.T0; G=str.H0; J=str.G0;
%  [Sigma,ferror] = mlyapunov(F,G*G',.995);
% end
% %check invertibility
% FKH=F-(G/L)*H;
