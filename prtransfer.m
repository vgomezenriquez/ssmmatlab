...
%
% This function computes the right transfer function K(z)=B(z)*A^{-1}(z) up to
...
[s3, s4, nq] = size(B);
if (s1 ~= s2) | (s1 ~= s4)
    disp('wrong dimensions of A or B in prtransfer');
    ierror = 1;
    return
end
p = np - 1;
...
for j = 1:n - 1
    ...
    for i = 1:min(j, p)
        if (i < j)
            K(:, :, j+1) = K(:, :, j+1) - K(:, :, j-i+1) * A(:, :, i+1);
        else
            K(:, :, j+1) = K(:, :, j+1) - K0 * A(:, :, i+1);
        end
    end
    if inv == 1
        K(:, :, j+1) = K(:, :, j+1) / A0;
