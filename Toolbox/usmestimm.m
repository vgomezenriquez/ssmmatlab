...
%
% This function estimates a univariate structural model with complex seasonal
% patterns using the exact maximum likelihood method.
%
%
% Inputs:
...
[xvp, J, ff, g, iter] = marqdt(infm, xv, y, pfix, pvar, xf, chb, str); % xvp,pause
maxabsxvidx = 0;
maxabsxv = -1.d10;
stord = str.stord;
for i = 1:nr
...
if (maxabsxvidx > 0) %change of concentration parameter
    ...
    cc = cell(1, 5+N);
    cc(1:3) = {'str.comp.irreg', 'str.comp.level', 'str.comp.slope'};
    for i = 1:N
        cc{3+i} = ['str.comp.seasp{', num2str(i), '}'];
    end
    cc(4+N:5+N) = {'str.comp.ar', 'str.comp.cycle'};
    oldc = eval(cc{oldconc});
    %    oldc,pause
    if (oldconc >= 4) && (oldconc <= 3 + N)
        oldx = oldc(3);
    else
        oldx = oldc(2);
    end
    ...
    str.x(npvar) = xv;
    str.x(npfix) = xf;
    [X, Z, G, W, T, H, ins, ii, ferror] = pr2usmm(xv, xf, str);
    str.X = X;
    str.Z = Z;
    str.G = G;
    str.W = W;
...
if (maxabsxvidx > 0) || ((maxabsxvidx == 0) && (iter == maxit0))
    ...
    [xvf, J, ff, g, iter, conf] = marqdt(infm, xv, y, pfix, pvar, xf, chb, str);
...
%
[F, e, h, M, Pevf, A, P] = smfunm(xvf, y, pfix, pvar, xf, chb, str);
