...
if ~isempty(x)
    [nobs2, nx] = size(x);
    if (nobs2 ~= nobs)
        error('nselimhr3: nobs in x-matrix not the same as y-matrix');
    end
else
    nx = 0;
end
...
kro = str.kro;
phitv = str.phitv3;
thetatv = str.thetatv3;
gammatv = str.gammatv3;

if (s ~= neqs)
    error('nselimhr3: s nonequal dim(y_t)');
end
if (m ~= nx)
    error('nselimhr3: m nonequal dim(x_t)');
end
            ...
            if (isnan(gamma(i, j, k)) && (tval < mint))
                ...
                if k == kro(i) + 1 %enforce row degrees
                    ...
                    for jj = 1:jt
                        ...
                        %        if (isnan(phi(i,jj,k)) | isnan(theta(i,jj,k)))
                        %         nkro=0;
                        %        end
                    end
                end
