...
% parameters possibly restricted to zero using the exact maximum likelihood
% method with a fast square root filter. It is assumed that initial values
% obtained with the three stages of the Hannan-Rissanen or the conditional
% method are available in str.beta3. These initial estimates can be
% obtained using the functions MHANRIS, ESTVARMAXPQRPQR or ESTVARAMXKRO for
% the Hannan-Rissanen method or function MCONESTIM for the conditional
...
% LLp=L*L';mkro=max(str.kro);
% phi=str.phis3;
% theta=str.thetas3;
% %compute covariances of y
% [c,ierror]=macgf(phi,theta,LLp,mkro+1);c
% [css,ierror]=macgfss(str.Fs3,str.Ks3,str.Hs3,LLp,LLp,LLp,mkro+1);css
% return
% [ff,beta,e,f,str]=exactmedfv(xv,y,x,str,tol); e(1:5,:),f
% X=[]; W=[]; Z=str.Hs3; G=L; T=str.Fs3; H=str.Ks3*L;
% [Omega,ferror] = mlyapunov(T,H*H');
% i=[sum(str.kro) 0 0 0]; ins=[Omega]; chb=0;
% [e,f,g,M]=scakfle2(y,X,Z,G,W,T,H,ins,i,chb);
% F=e*f; e(1:5,:),f
% return
...
[ff, xvft, e, f, str, hb, Mb] = exactmedfv(xvf, y, x, str, tol, Y, chb);
