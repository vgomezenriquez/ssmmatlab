...
%
% this function automatically estimates the duration period of the Easter
% effect for an ARIMA model with Easter correction
%
% Input arguments:
...
% durval : an integer array containing the possible days previous to Easter
%         (0 is also a value)
...
if ~isstruct(parm)
    error('durid: requires a parameter structure');
end;
oparm = parm;
ny = length(y);
s = parm.s;
...
for i = 1:ndur
    dur = durval(i);
    if dur > 0
        Ye = east(bg_year, bg_per, ny, dur, freq, Yd);
        YY = [Y, Ye];
    else
        YY = Y;
    end
    ...
    xf = x0(pfix);
    if ~isempty(xv)
        xv = arimaopt(fmarqdt, fid, x0, xv, xf, y, YY, parm, infm, 0); %comment this line to use HR method
        x0(pvar) = xv;
