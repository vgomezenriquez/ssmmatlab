function [KKP, PT, recrs, recr, srecr, t1, A1, LP1, KG] = scakfffsqrt(y, X, Z, G, W, T, H, ins, i, g, icollps)
%
%
...
%        residuals. The regression coefficient, g, is considered fixed.
%
...
nnalpha = n * nalpha;
if nbeta > np
    disp('too many regressors in scakfffsqrt')
    return
end
if mi ~= nalpha
...
for i = 1:n
    ...
    if miss == p
        A = [zeros(nalpha, ndelta), Wg] + TT * A; %x_{t+1|t}
        LP = RR(1:nlp, 1:nlp)'; %P_{t+1}^{1/2}
    else
        A = [zeros(nalpha, ndelta), Wg] + TT * A + whK * DD; %x_{t+1|t}
    ...
    if collps == 0 && idxi >= nd
        ...
        if (rank(LL) == ndelta) && (i > icollps)
            ...
            if (nomiss > nd)
                ...
                ets = SQT(nd1:idxi, end); %standardized recursive residuals
                nets = size(ets, 1);
                nc = size(Frt, 2);
                %
