...
if iar > 1
    fprintf(1, 'initial model nonstationary in mexactestimc, iar = %2d\n', iar);
    str = sta3r(str);
end
ima = chkstainv(str.Fs3-str.Ks3*str.Hs3); %if ima >1, the model is not invertible
if ima > 1
    fprintf(1, 'inital model noninvertible in mexactestimc, ima = %2d\n', ima);
    str = inv3r(str);
end
...
L = R';
if (p > 0)
    disp('covariance matrix of residuals singular in mexactestimc')
    ferror = 1;
    return
end
...
if (abs(sum(sum(J))) <= 1.d-8)
    error('estimation failed in mexactestimc')
end
%end of addition
beta = xvf';
