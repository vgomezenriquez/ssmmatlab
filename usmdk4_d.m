...
% example in the second edition of this book (2012), p. 195. I believe the
% correct results are those of the first edition, which roughly coincide
% with the results obtained with this script file.
% In this case, the covariance matrix of the seasonal disturbance is set to
% zero.
%
%
...
Y = data(1:end-nb, 4:5);
% Y=[];
%add intervention (level shift)
if (nb == 0)
    ny = size(data, 1);
    step = [zeros(169, 1); ones(ny-169, 1)];
    Y = [Y, step];
end
...
tname = 'frontrear';
fname = fullfile('results', 'frontrear1.txt');
...
YY = [];
comp.level = [1, .1, NaN];
comp.seas = [2, .0, NaN]; % parameter zero, fixed seasonal
comp.irreg = [1, .1, NaN];
comp.conout = 'level';
comp.freq = freqm;
comp.datei = dateim;
...
x0 = [.1, .1, ... %sigma level s21 s22 (s11 one parameter is concentrated out)
    .0, .0, .0, ... %sigma seasonal s11 s21 s22; parameters zero, fixed seasonal
    .1, .1, .1, ... %sigma irreg. s11 s21 s22
    ];


pfix = [3:5]; %fixed parameters
pvar = [1:2, 6:8]; %free parameters
...
disp('More estimation and diagnostic details are in file "frontrear1.txt"')
